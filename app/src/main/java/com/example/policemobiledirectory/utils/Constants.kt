...
object Constants {
    ...
    private val baseCommonUnitNames = listOf(
        "Control Room", "DPO", "Computer Sec", "DAR", "FPB", "MCU", "DCRB", "DSB", "SMMC",
        "State INT", "DCRE", "Lokayukta", "ESCOM"
    )
    ...
    val stationsByDistrictMap: Map<String, List<String>> = districtsList.associateWith { districtName ->
        ...
        val generatedCommonUnits = baseCommonUnitNames.map { unit ->
            "$unit $districtName"
        }
        (specificStations + generatedCommonUnits + districtName).distinct().sorted()
    }
}
